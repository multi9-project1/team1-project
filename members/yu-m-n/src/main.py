...
MODEL_CONFIG = {
    "CatBoost": {
        "val_pred": "val_predictions_catb.csv",
        "feature_importance": "feature_importance_catb.csv",
        "test_pred": "test_predictions_catb.csv",
    },
    "ExtraTrees": {
        "val_pred": "val_predictions_extrees.csv",
        "feature_importance": "feature_importance_extrees.csv",
        "test_pred": "test_predictions_extrees.csv",
    },
    "KNN": {
        "val_pred": "val_predictions_knn.csv",
        "feature_importance": None,
        "test_pred": "test_predictions_knn.csv",
    },
    "LightGBM": {
        "val_pred": "val_predictions_lightgbm.csv",
        "feature_importance": "feature_importance_lightgbm.csv",
        "test_pred": "test_predictions_lightgbm.csv",
    },
    "Logistic Regression": {
        "val_pred": "val_predictions_logistic.csv",
        "feature_importance": None,
        "test_pred": "test_predictions_logistic.csv",
    },
    "RandomForest": {
        "val_pred": "val_predictions_rf.csv",
        "feature_importance": "feature_importance_rf.csv",
        "test_pred": "test_predictions_rf.csv",
    },
    "XGBoost": {
        "val_pred": "val_predictions_xgboost.csv",
        "feature_importance": "feature_importance_xgboost.csv",
...
def load_data():
    ...
    for model_name, cfg in MODEL_CONFIG.items():
        val_path = BASE_DIR / cfg["val_pred"] if cfg["val_pred"] is not None else None
        fi_path = BASE_DIR / cfg["feature_importance"] if cfg["feature_importance"] is not None else None
        test_path = BASE_DIR / cfg["test_pred"] if cfg["test_pred"] is not None else None

        model_files[model_name] = {
            "val_pred": load_csv_if_exists(val_path),
            "feature_importance": load_csv_if_exists(fi_path),
            "test_pred": load_csv_if_exists(test_path),
        }

    return x_val, y_val, model_files
...
# -----------------------------
def build_validation_df(x_val, y_val, pred_df):
    if pred_df is None:
        return None

    if not REQUIRED_PRED_COLS.issubset(pred_df.columns):
        raise ValueError("예측 파일에는 pred_prob, pred_label 컬럼이 있어야 합니다.")

    if "returned" not in y_val.columns:
        raise ValueError("y_val.csv에는 returned 컬럼이 있어야 합니다.")

    if len(x_val) != len(y_val) or len(x_val) != len(pred_df):
        raise ValueError("X_val_unscaled.csv, y_val.csv, 예측 파일의 행 개수가 서로 다릅니다.")

    df = x_val.copy()
    df["returned"] = y_val["returned"].values
    df["pred_prob"] = pred_df["pred_prob"].values
...
# 필터 적용
# -----------------------------
def apply_filters(df, selected_category, selected_shipping, selected_risk, price_range):
    if df is None:
        return None
    ...
    if selected_shipping != "전체":
        filtered = filtered[filtered["shipping_method_label"] == selected_shipping]
...
# 혼동행렬 그리기
# -----------------------------
def draw_confusion_matrix(df, title):
    cm = pd.crosstab(
        df["returned"],
        df["pred_label"],
        rownames=["Actual"],
    ...
    apply_korean_font(ax, title, "예측값", "실제값")
...
# -----------------------------
try:
    X_val, y_val, model_files = load_data()
...
for model_name, files in model_files.items():
    try:
        df = build_validation_df(X_val, y_val, files["val_pred"])
        if df is not None:
            all_val_dfs[model_name] = df
            available_models.append(model_name)
    except Exception as e:
        st.warning(f"{model_name} validation 데이터 구성 중 오류: {e}")

if not available_models:
    st.error("사용 가능한 validation 예측 파일이 없습니다.")
    st.stop()
...
st.caption("Validation 예측 결과와 변수 중요도를 기반으로 반품 패턴, 모델 성능, 혼동행렬을 비교합니다.")
...
base_df_for_options = all_val_dfs[available_models[0]]

category_options = ["전체"] + sorted(base_df_for_options["product_category_label"].dropna().unique().tolist())
shipping_options = ["전체"] + sorted(base_df_for_options["shipping_method_label"].dropna().unique().tolist())
...
selected_model = st.sidebar.selectbox("분석 모델 선택", available_models)
selected_importance_model = st.sidebar.selectbox("중요 변수 모델 선택", available_models)

price_range = st.sidebar.slider(
    "상품 가격 범위",
    min_value=int(base_df_for_options["product_price"].min()),
    max_value=int(base_df_for_options["product_price"].max()),
    value=(int(base_df_for_options["product_price"].min()), int(base_df_for_options["product_price"].max()))
)

filtered_model_dfs = {
    model_name: apply_filters(df, selected_category, selected_shipping, selected_risk, price_range)
    for model_name, df in all_val_dfs.items()
}

filtered_df = filtered_model_dfs[selected_model]
...
k0.metric("선택 모델", selected_model)
...
for model_name in available_models:
    acc, f1, prec, rec = calculate_metrics(filtered_model_dfs[model_name])
...
with c_right:
    ...
    comparison_df.set_index("model")[["accuracy", "f1", "precision", "recall"]].plot(kind="bar", ax=ax)
    apply_korean_font(ax, "모델별 성능 비교", "모델", "점수")
...
with row1_col1:
    ...
    apply_korean_font(ax, f"{selected_model} 예측 확률 분포", "예측 확률", "주문 수")
    st.pyplot(fig)
...
with row1_col2:
    ...
    apply_korean_font(ax, f"{selected_model} 리스크 레벨 분포", "리스크 레벨", "주문 수")
    st.pyplot(fig)
...
# 선택 모델 혼동행렬
# -----------------------------
st.subheader("선택 모델 혼동행렬")

cm, cm_fig = draw_confusion_matrix(filtered_df, f"{selected_model} 혼동행렬")

col_cm1, col_cm2 = st.columns([1, 1.3])
...
importance_df = model_files[selected_importance_model]["feature_importance"]
...
if importance_df is not None and {"feature", "importance"}.issubset(importance_df.columns):
    ...
    apply_korean_font(ax, f"{selected_importance_model} Feature Importance TOP 10", "Importance", "")
    st.pyplot(fig)
else:
    st.warning(f"{selected_importance_model} feature importance 파일이 없거나 컬럼 형식이 맞지 않습니다.")
...
test_selected_model = st.selectbox("Test 예측 결과 모델 선택", available_models)
selected_test_pred = model_files[test_selected_model]["test_pred"]

if selected_test_pred is not None and REQUIRED_PRED_COLS.issubset(selected_test_pred.columns):
    t1, t2, t3 = st.columns(3)
    t1.metric("Test 예측 건수", f"{len(selected_test_pred):,}")
    t2.metric("평균 예측 확률", f"{selected_test_pred['pred_prob'].mean():.1%}")
    t3.metric("고위험 예측 비율", f"{(selected_test_pred['pred_prob'] >= 0.7).mean():.1%}")

    fig, ax = plt.subplots(figsize=(6, 4))
    ax.hist(selected_test_pred["pred_prob"], bins=20)
    ax.axvline(0.7, linestyle="--")
    apply_korean_font(ax, f"{test_selected_model} Test 예측 확률 분포", "예측 확률", "주문 수")
    st.pyplot(fig)
else:
    st.info(f"{test_selected_model} test 예측 파일이 없거나 pred_prob, pred_label 컬럼이 없습니다.")
