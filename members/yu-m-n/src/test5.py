...
MODEL_CONFIG = {
    "Logistic Regression": {
        "val_pred": "val_predictions.csv",
        "feature_importance": "feature_importance.csv",
        "test_pred": "test_predictions.csv",
    },
    "LightGBM": {
        "val_pred": "val_predictions_lgbm.csv",
        "feature_importance": "feature_importance_lgbm.csv",
        "test_pred": "test_predictions_lgbm.csv",
    },
    "RandomForest": {
        "val_pred": "val_predictions_ranf.csv",
        "feature_importance": "feature_importance_ranf.csv",
        "test_pred": "test_predictions_ranf.csv",
    },
    "XGBoost": {
        "val_pred": "val_predictions_xgb.csv",
        "feature_importance": "feature_importance_xgb.csv",
        "test_pred": "test_predictions_xgb.csv",
    },
    "CatBoost": {
        "val_pred": "val_predictions_cat.csv",
        "feature_importance": "feature_importance_cat.csv",
        "test_pred": "test_predictions_cat.csv",
    },
}
...
def load_data():
    ...
    for model_name, cfg in MODEL_CONFIG.items():
        model_files[model_name] = {
            "val_pred": load_csv_if_exists(BASE_DIR / cfg["val_pred"]),
            "feature_importance": load_csv_if_exists(BASE_DIR / cfg["feature_importance"]),
            "test_pred": load_csv_if_exists(BASE_DIR / cfg["test_pred"]),
        }
...
def apply_korean_font(ax, title=None, xlabel=None, ylabel=None):
    if title is not None:
        ax.set_title(title, fontproperties=font_prop) if font_prop is not None else ax.set_title(title)
    if xlabel is not None:
        ax.set_xlabel(xlabel, fontproperties=font_prop) if font_prop is not None else ax.set_xlabel(xlabel)
    if ylabel is not None:
        ax.set_ylabel(ylabel, fontproperties=font_prop) if font_prop is not None else ax.set_ylabel(ylabel)
...
with c_left:
    st.dataframe(comparison_df.sort_values("f1", ascending=False), use_container_width=True, hide_index=True)
